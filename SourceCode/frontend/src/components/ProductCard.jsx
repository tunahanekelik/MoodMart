...
// New props from App.js: onToggleWish and wishlist added
  ...
  return (
    <>
      <Card sx={{ 
        ...
        overflow: 'hidden', transition: '0.3s', position: 'relative', // Relative position for heart button
        ...
        <IconButton 
          onClick={(e) => {
            e.stopPropagation(); // Prevents other card click events
        ...
        >
          {isWished ? (
            <Favorite sx={{ color: '#ff5252' }} /> // Red heart if in wishlist
          ) : (
            <FavoriteBorder sx={{ color: '#fff' }} /> // Empty heart if not in wishlist
          )}
        </IconButton>
        ...
        <CardMedia
            ...
            onError={(e) => { e.target.src = 'https://via.placeholder.com/500x300?text=Product+Coming+Soon'; }}
        />
