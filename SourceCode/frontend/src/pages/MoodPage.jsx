...
// onAddToCart prop should come from App.js
const MoodPage = ({ onAddToCart, onToggleWish, wishlist }) => {
  const { moodName } = useParams();
  const [products, setProducts] = useState([]);

  useEffect(() => {
    // Fetching products related to the specific mood from the Backend
          ...
          {products.length > 0 ? (
            products.map(product => (
              <Grid item xs={12} sm={6} md={4} key={product.id}>
                {/* Prop Drilling: onAddToCart is passed down to ProductCard. 
                   Path: App.js -> MoodPage -> ProductCard
                */}
                <ProductCard 
                  product={product} 
