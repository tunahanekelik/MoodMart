...
// Custom mood images located in the public folder
const moodImages = [
...
// Map backend status strings to stepper indices for progress tracking
const statusMapping = { 
  "PLACED": 0, 
  "PREPARING": 1, 
  "SHIPPED": 2, 
...
const ProfilePage = ({ wishlist, onRemoveWish }) => {
  ...
  // Fetch user-specific orders when the tab changes or user ID updates
  useEffect(() => {
    if (activeTab === 'orders') {
  ...
  // Update profile picture in state and persistent local storage
  ...
  // Handle local file upload for custom profile pictures
        ...
        <Grid container spacing={4} alignItems="stretch">
          ...
          <Grid item xs={12} md={8}>
            <Paper sx={{ 
              ...
              {/* Settings Tab: Profile customization options */}
              {activeTab === 'settings' && (
                <Box>
                  ...
                  <Grid container spacing={2} sx={{ mb: 4 }}>
                    ...
                    {/* Display preset mood icon selection */}
                    {moodImages.map((m) => (
              ...
              {/* Wishlist Tab: View and manage saved products */}
              {activeTab === 'wishlist' && (
                <Box>
              ...
              {/* Orders Tab: Track shipment and order status */}
              {activeTab === 'orders' && (
                <Box>
                  ...
                  {orders.length > 0 ? orders.map(order => (
                    <Card key={order.id} sx={{ mb: 3, bgcolor: '#252525', color: 'white', borderRadius: 4 }}>
                      ...
                      <Box sx={{ p: 4 }}>
                        {/* Stepper showing real-time progress based on backend order status */}
                        <Stepper activeStep={statusMapping[order.status?.toUpperCase()]} alternativeLabel>
